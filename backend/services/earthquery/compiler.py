...
_INTENT_PATTERNS: list[tuple[str, str, str]] = [
    ...
    (r"(show me (the )?change|detect change|change map|changes between|difference between|before.?after|temporal change)", "change_detection", "Bi-temporal Change Detection"),
    (r"(affected|impact|impacted|damage|damaged|loss|submerged|destruction|destroyed|inundated|hazard|प्रभावित|नुकसान|असर|kya asar|kitna asar|kitna nuksan|kitna.*affect)", "change_vqa", "Change-VQA"),
    (r"(what (happened|changed)|how (much|many).*(changed|different|affected|lost)|कितना.*(बदला|नुकसान|अंतर|परिवर्तन)|क्या बदलाव|क्या बदला|kya badla|kitna badlav|kya hua|kitna nuksan|kitna asar)", "change_vqa", "Change-VQA"),
    (r"(change|changed|difference|deforest|urban.?sprawl|बदलाव|परिवर्तन|अंतर|बाढ़|पहले.*बाद|badlav|parivartan|pehle.*baad|farq|kya difference|compare)", "change_detection", "Bi-temporal Change Detection"),
    ...
    (r"(what|how many|how much|is there|are there|count|classify|label|क्या|कितने|कितना|kya|kitne|kitna|hai kya|kya ye)", "vqa", "Visual Question Answering"),
]
...
_ENTITY_PATTERNS = [
    ...
    r"\b(ndvi|ndwi|nbr|spectral|multispectral|reflectance|sar|radar)\b",
    r"\b(affected|damage|impact|loss|hazard|submerged|inundated|land|area|canopy|forest)\b",
    r"(बाढ़|आग|जंगल|इमारत|भवन|सड़क|नदी|झील|बादल|बर्फ|वनस्पति|फसल|मिट्टी|पेड़|प्रभावित|नुकसान|असर|क्षति|जलमग्न|ज़मीन|भूमि)",
...
def compile_query(question: str, has_two_images: bool = False) -> EarthQuerySpec:
    ...
    # If user provided two images and asked a question, promote generic VQA to Change-VQA
    if has_two_images and task_type in ("vqa", "unknown"):
        task_type = "change_vqa"
        intent = "Change-VQA"

    # If user only provided one image, downgrade change_vqa to single-image vqa
    # unless it explicitly requires comparison across dates
    if not has_two_images and task_type in ("change_vqa", "change_detection"):
        is_strictly_bitemporal = bool(re.search(r"before.*after|pehle.*baad|difference between|compare|change between|between.*and|between t0|t0.*t1|20\d\d.*20\d\d|पहले.*बाद", q))
